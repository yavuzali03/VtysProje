export class HomeScreenData {
  static async getTopPlayerData(setTopPlayer, setLoading) {
    setLoading(true);
    try {
  ...
  static async getMostLikedPlayerData(setTopPlayer) {

    try {
      const response = await fetch(`http://44.195.206.105/get-player-like`);
      const json = await response.json();
      if (json.player_id === "313131313131") {
        json.image_url = "https://i.ibb.co/ggrKT1V/vesikal-k.png";
      } else if (json.player_id === "464646464646") {
